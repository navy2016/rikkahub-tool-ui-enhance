...
class GenerationHandler(
                ...
                when (tool.approvalState) {
                    ...
                    is ToolApprovalState.Pending -> {
                        ...
                        runCatching {
                            ...
                            // Apply default parameters fallback when LLM generates invalid/empty parameters
                            val parsedArgs = tool.input.ifBlank { null }
                            val isEmptyArgs = parsedArgs == null || 
                                (parsedArgs.trim().let { it == "{}" || it == "{ }" || it.isEmpty() })
                            val args = if (isEmptyArgs) {
                                Log.i(TAG, "generateText: tool ${toolDef.name} received empty/invalid params, using defaults")
    ...
    private suspend fun generateInternal(
        ...
        stream: Boolean,
        workflowPhase: me.rerere.rikkahub.data.model.WorkflowPhase? = null
    ) {
        val internalMessages = buildList {
            val system = buildString {
                ...
                if (workflowPhase != null) {
                    appendLine()
                    append(me.rerere.rikkahub.data.ai.buildWorkflowPhasePrompt(workflowPhase))
                }
