...
import me.rerere.rikkahub.data.document.epub.EpubTextParser
import java.io.File
...
data class DocumentExtractOptions(
    val enableImageOcr: Boolean = false,
)
...
object DocumentTextExtractor {
    ...
    private val officeExtensions = setOf(
        "pdf", "doc", "docx", "docm", "ppt", "pptx", "pptm", "xls", "xlsx", "xlsm", "epub"
    )
    ...
    fun streamText(
        ...
        fileName: String,
        mimeType: String?,
        options: DocumentExtractOptions = DocumentExtractOptions(),
        ...
        when (extensionOf(fileName)) {
            ...
            "epub" -> EpubTextParser.stream(file, options, sink)
        ...
        file.bufferedReader().use { reader ->
            ...
            while (true) {
                ...
                if (buffer.length >= TEXT_BLOCK_SIZE) {
                    progress += 1
                    if (!emitNormalized(buffer.toString(), progress, 0, "文本块", sink)) return
                    buffer.clear()
                }
            }
